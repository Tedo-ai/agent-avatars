 ...
 *   wasBackground – true on the first frame after the tab becomes visible again
 */
...
if (typeof document !== 'undefined') {
  document.addEventListener('visibilitychange', () => {
    ...
    if (!hidden && subscribers.size > 0 && frameId === null) {
      // Tab just became visible — restart the loop
...
function tick(timestamp) {
  // Don't render while hidden — freeze everything
  if (hidden) {
    frameId = null;
    return;
  }
