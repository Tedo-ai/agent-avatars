 ...
 * @param {object} [opts]
 * @param {number} [opts.hue] - Override hue (0-360). If provided, uses this instead of seed-derived hue.
 */
export function generatePalette(seed, mode, opts) {
  const rng = seededRandom(seed);
  const hue = opts?.hue != null ? (opts.hue % 360) : Math.floor(rng() * 360);
