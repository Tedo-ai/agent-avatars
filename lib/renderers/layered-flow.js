...
export function create(container, seed, size, mode) {
  ...
  const sizes = [1, 1.5, 2.2], alphas = [0.3, 0.5, 0.7], counts = [50, 40, 30];
  ...
  function startLoop() {
    ...
    unsub = subscribe((ts) => {
      ...
      if (energy === 0 && !active) {
        layers.forEach(parts => { for (const p of parts) p.trail = []; });
        drawStatic();
      ...
      layers.forEach((parts, li) => {
        const col = layerColors[li], sz = sizes[li], al = alphas[li];
        for (const p of parts) {
          const lifeAlpha = Math.min(p.life / 4, 1) * Math.min((p.maxLife - p.life) / 4, 1) * al * Math.max(e, 0.05);
          for (let i = 0; i < p.trail.length; i++) {
            const freshness = (i + 1) / p.trail.length;
            const a = lifeAlpha * freshness;
            if (a < 0.01) continue;
            bctx.beginPath();
            bctx.arc(p.trail[i].x, p.trail[i].y, sz, 0, Math.PI * 2);
            bctx.fillStyle = `rgba(${col[0]},${col[1]},${col[2]},${a})`;
            bctx.fill();
          }
        }
      });
