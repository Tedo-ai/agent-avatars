...
export function createCanvasManager(container, seed, size, drawFn, opts) {
  ...
  function onFrame(timestamp) {
    const rawDt = lastTimestamp !== null ? timestamp - lastTimestamp : 16;
    lastTimestamp = timestamp;
    // If gap > 200ms the tab was backgrounded — just resume, don't fast-forward
    const dt = rawDt > 200 ? NOMINAL_DT : Math.min(rawDt, 50);
